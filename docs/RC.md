...
1. **1-A. OSV 結果のキャッシュ** — 即効性大、実装小、既存資産流用可
2. **1-C. 検索 / フィルタ** — UX 体感が大きく変わる、実装小
3. **1-B. 依存ツリー可視化** — 既に持っている親追跡情報の自然な発展
4. **2-B. deprecation / ライセンス表示** — registry アクセスの拡張で済む
5. **4-A. 複数選択での一括更新** — 既存ロジックの軽微な拡張
